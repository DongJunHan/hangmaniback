...
public class StoreQueryConst {
    ...
    public static final String getStoreInfoWithWinCountBySidoSigugun = "SELECT s.storeUuid, s.storeName, s.storeaddress, \n" +
...
//            "COALESCE(GROUP_CONCAT(DISTINCT l.lottoname ORDER BY l.lottoname ASC SEPARATOR ', '),'') AS lottonames, \n"+
            "COALESCE(GROUP_CONCAT(DISTINCT sa.saved_file_name ORDER BY sa.saved_file_name ASC SEPARATOR ', '), '') AS saved_file_names, \n" +
            ...
            "GROUP BY s.storeUuid, s.storeName, s.storeaddress, s.storelatitude, s.storelongitude\n";
    public static final String getStoreInfoWithWinCountByLatitudeLongitude = "SELECT s.storeUuid, s.storeName, s.storeaddress, " +
            "SUM(CASE WHEN w.winRank = 1 THEN 1 ELSE 0 END) AS win1stCount, " +
            "SUM(CASE WHEN w.winRank = 2 THEN 1 ELSE 0 END) AS win2stCount, " +
            "s.storelatitude, s.storelongitude, "+
//            "COALESCE(GROUP_CONCAT(lh.lottoname  ORDER BY lh.lottoname ASC SEPARATOR ', '),'') AS lottonames, "+
            "COALESCE(GROUP_CONCAT(DISTINCT sa.saved_file_name ORDER BY sa.saved_file_name ASC SEPARATOR ', '), '') AS saved_file_names, " +
            "get_distance(?, ?, s.storelatitude, s.storelongitude) AS distance "+
            "FROM " +
            "    store s " +
